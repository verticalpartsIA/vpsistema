...
export default function ModuleCard({ module: mod, locked = false, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`group relative bg-surface-card border border-surface-border rounded-2xl p-6 text-left
    ...
    >
      ...
      {/* Ícone — sempre o ícone do módulo, nunca cadeado */}
      <div
      ...
      <div className="mt-4 flex items-center gap-1 text-xs font-medium" style={{ color }}>
        {locked ? (
          <span className="text-slate-600">Sem permissão</span>
        ) : (
